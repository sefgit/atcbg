...
var atps = [];
var cards=[];
var limits = {"acard":15,"tcard":18,"ccard":16,"gcard":15};
var questions = {"acard":[],"tcard":[],"ccard":[],"gcard":[]};
...
function startToPlay()
{
    ...
    setTimeout(function() {
		...
		setTimeout(function() {
			...
			el.style.display = 'block';
			spinner = new PlainDraggable(el);
			
			var els = ['power', 'event'];
			var len = [5,6];
			for(var x in els) {
				var e = els[x];
				var arr = shuffleInts(len[x]);
				for(var k in arr) {
					var i = arr[k];
					var name = e+i;
					var tag = document.createElement("div");
					tag.id = name;
					tag.className = "card "+e+"card "+name+" hidden";
					...
					el.ondblclick=function() {
						...
						if (this.classList.contains(n)) {
							...
							this.classList.add(n+'b');
						} else {
							this.classList.remove(n+'b');
							this.classList.add(n);
						}
					}
					cards.push(new PlainDraggable(el));
				}
			}
			els = ['acard', 'tcard', 'ccard', 'gcard'];
			for(var i in els) {
				var e = els[i];
				...
				el.ondblclick=function() {
					...
					if (this.classList.contains(n)) {
						this.classList.remove(n);
						this.classList.add(n+'b');
						this.classList.add('qnr');
						var limit = limits[n];
						var k = '0'+(Math.random()*limit+1);
						var arr = questions[n];
						if (arr.length > 0) {
							var x = arr.shift();
							k = '0'+x;
						}
						this.innerHTML = k.substring(k.length -2, k.length);
					} else {
						this.classList.remove('qnr');
						this.classList.remove(n+'b');
						this.classList.add(n);
						this.innerHTML='';
					}
